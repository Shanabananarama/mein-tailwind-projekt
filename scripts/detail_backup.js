    ...
    .then((data) => {
      ...
      if (card) {
        document.getElementById("card-title").textContent =
          `${card.name} – ${card.series} Trading Card`;
        document.getElementById("card-series").textContent =
          `Serie: ${card.series}`;
        document.getElementById("card-description").textContent =
          `${card.description} – limitierte Edition.`;
        ...
        document.getElementById("card-details").innerHTML =
          "<p class='text-red-600 font-bold'>Fehler beim Laden der Kartendetails.</p>";
      }
    })
    .catch((error) => {
